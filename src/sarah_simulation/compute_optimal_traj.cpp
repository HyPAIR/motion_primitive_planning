...
std::vector<double> GenerateOptimalTimeProfileSegment(const std::vector<double> &stations, double start_time, std::shared_ptr<liom_local_planner::PlannerConfig> config_) {
...
FullStates ResamplePath(const std::vector<math::Pose> &path, std::shared_ptr<liom_local_planner::PlannerConfig> config_) {
...
int main(int argc, char* argv[]) {
    ...
    while (ros::ok()) {
      ...
      writeTrajectoryToYAML(sol_all, "data");
      writetime_count(time_set, "data");
      ROS_WARN("Successfully generate the optimal trajectory!");
      break;
      ros::spinOnce();
      rate.sleep();	
	}
