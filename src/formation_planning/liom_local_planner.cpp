...
bool LiomLocalPlanner::Plan(const FullStates &prev_sol, const TrajectoryPoint &start, const TrajectoryPoint &goal, 
      iris::IRISProblem &iris_problem, FullStates &result) {
  ...
  while (!CheckDiffDriveKinematic(result, offset, re_phi) && !CheckCarKinematic(result, offset_car)) {
    ...
    problem_->Solve(config_->opti_w_penalty0, constraints, guess, result, infeasibility);
  }
...
bool LiomLocalPlanner::Plan_car_like(const FullStates &traj_lead, const double offset, FullStates &traj_follower) {
  double dt = traj_lead.tf / traj_lead.states.size();
	...
	for (size_t l_index = 1; l_index < traj_lead.states.size(); l_index++) {
...
bool LiomLocalPlanner::Plan_diff_drive(const FullStates &guess, const FullStates &prev_sol, const TrajectoryPoint &start, const TrajectoryPoint &goal, 
  FullStates &result, const int robot_index, double& infeasibility) {
  Constraints constraints;
  constraints.start = start;
  constraints.goal = goal;
  double max_error;
  ...
  if(!problem_->Solve_diff_drive(config_->opti_w_penalty0, constraints, guess, prev_sol, result, infeasibility)) {
    ROS_ERROR("solver failed!");
    return false;
  }
...
bool LiomLocalPlanner::Plan_car_like_replan(const FullStates &guess, const FullStates &prev_sol, FullStates &result, const int robot_index, double& infeasible) {
  Constraints constraints;
  double infeasibility;
  double max_error = 0.0, avg_error = 0.0, error_time = 0.0;
  if(!problem_->Solve_replan(config_->opti_w_penalty0, constraints, guess, prev_sol, result, infeasibility)) {
    ROS_ERROR("solver failed!");
    return false;
  }
...
bool LiomLocalPlanner::CheckCarKinematic(const FullStates &current_result, const std::vector<double> offset_car) {
...
bool LiomLocalPlanner::CheckDiffDriveKinematic(const FullStates &current_result, const std::vector<double> offset, const std::vector<double> re_phi) {
