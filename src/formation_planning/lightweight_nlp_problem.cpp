...
public:
  ...
  virtual bool get_starting_point(Ipopt::Index n, bool init_x, Ipopt::Number* x,
    ...
    for(int i = 0; i < nrows_; i++) {
      ...
      auto x0_disc = config_.vehicle.GetVertexPositions(guess_.states[i+1].x, guess_.states[i+1].y, guess_.states[i+1].theta);
...
bool LightweightProblem::Solve(
  ...
  std::cout << "car-like robot"
    << "wall_t: " << GetCurrentTimestamp() - solver_st
    << ", status: " << status
...
bool LightweightProblem::Solve_diff_drive(
  double w_inf, const Constraints &profile, const FullStates &guess, const FullStates &pre_sol, FullStates &result, double &infeasibility) {
  double solver_st = GetCurrentTimestamp();
