...
public:
  ...
  template<class T>
  std::vector<T> GetVertexPositions(const T &x, const T &y, const T &theta) const {
    ...
    result.push_back(vert_l_y + (1.2 + offset) * sin(theta));
    result.push_back(vert_l_x + hypot(1.2 + 2.0, 1.2 + offset) * cos(theta - atan2(1.2 + offset, 1.2 + 2.0)));
    result.push_back(vert_l_y + hypot(1.2 + 2.0, 1.2 + offset) * sin(theta - atan2(1.2 + offset, 1.2 + 2.0)));
    result.push_back(vert_l_x + (1.2 + offset) * cos(M_PI / 2 - theta));
    result.push_back(vert_l_y - (1.2 + offset) * sin(M_PI / 2 - theta)); 
    return result;
  }
